<?php
    namespace KSeven\License\System;
	...
	class BaseLib {
		...
		public function __construct(array $Date, bool $SSL)
		{ 
			$this->product_id = $Date["PRODUCTID"];
			$this->api_url = ($SSL ? 'http://' : 'http://') . $Date["URL"] . '/';
			$this->api_key = $Date["KEY"];
			$this->api_language = $Date["LANGUAGE"];
			$this->current_version = $Date["VERSION"];
			$this->verify_type = $Date["ENVATO"] ? 'non_envato' : 'envato';
			$this->verification_period = $Date["VARIFICATION"];

			$this->current_path = $Date["PATH"];
			$this->root_path = realpath($this->current_path.'/..');
			$this->license_file = $this->current_path.'/../license.key';
		...
		public function verify_license($time_based_check = false, $license = false, $client = false)
		{
			...
			if($time_based_check && $this->verification_period > 0){
				...
				$today = date('d-m-Y');
				if(empty($_SESSION["2e299e9b700945f"])){
					$_SESSION["2e299e9b700945f"] = '00-00-0000';
				}
				if($type == 1){
					$type_text = '1 day';
				}elseif($type == 3){
				...
				if(strtotime($today) >= strtotime($_SESSION["2e299e9b700945f"])){
					$get_data = $this->call_api(
						'POST',
